...
export default {
  ...
  async execute(interaction: ChatInputCommandInteraction, queryOptionName = "playlist") {
    ...
    if (!channel)
      return interaction.reply({ content: i18n.__("playlist.errorNotChannel"), ephemeral: false }).catch(console.error);
    ...
    try {
      ...
      if (interaction.replied)
        ...
        return interaction
          .reply({ content: i18n.__("playlist.errorNotFoundPlaylist"), ephemeral: false })
          .catch(console.error);
    }
