...
export default {
  ...
  execute(interaction: ChatInputCommandInteraction) {
    ...
    if (!queue)
      return interaction.reply({ content: i18n.__("volume.errorNotQueue"), ephemeral: false }).catch(console.error);

    if (!canModifyQueue(guildMemer!))
      return interaction.reply({ content: i18n.__("volume.errorNotChannel"), ephemeral: false }).catch(console.error);
    ...
    if (isNaN(volumeArg))
      return interaction.reply({ content: i18n.__("volume.errorNotNumber"), ephemeral: false }).catch(console.error);

    if (Number(volumeArg) > 100 || Number(volumeArg) < 0)
      return interaction.reply({ content: i18n.__("volume.errorNotValid"), ephemeral: false }).catch(console.error);
