...
export default {
  ...
  execute(interaction: ChatInputCommandInteraction) {
    ...
    if (!queue)
      return interaction.reply({ content: i18n.__("skipto.errorNotQueue"), ephemeral: false }).catch(console.error);

    if (!canModifyQueue(guildMemer!)) return i18n.__("common.errorNotChannel");

    if (playlistSlotArg > queue.songs.length)
      return interaction
        .reply({ content: i18n.__mf("skipto.errorNotValid", { length: queue.songs.length }), ephemeral: false })
        .catch(console.error);
